...
public class IgnoreValuesJsonComparator implements JsonComparator {

  private List<String> propertyValuesIgnored = new ArrayList<String>();

  public IgnoreValuesJsonComparator(List<String> propertyValuesIgnored) {
    this.propertyValuesIgnored = propertyValuesIgnored;
  }
  ...
  private boolean includesJson(JsonElement actualJson, JsonElement expectedJson) {
    ...
    if (expectedJson.isJsonObject()) {
      ...
      for (Entry<String, JsonElement> entry : expectedJson.getAsJsonObject().entrySet()) {
        ...
        if (!propertyValuesIgnored.contains(property)) {
